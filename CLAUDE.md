...
- **`server/discover` is spec-mandated ("Servers MUST implement it"), not optional or Claude-Code-specific.** A real `claude-code/2.1.240` client probes with it before anything else on connect. Our server didn't implement it at all through EMCP-007 — `claude mcp add` failed outright, and the client fell back to a legacy `initialize` handshake (protocol version 2025-11-25) that this server also doesn't speak (solution.md §3 deliberately builds no `initialize`). Confirmed live, not assumed: captured the real request/response pair before fixing. Fixed by registering `server/discover` in `MethodRegistry`, returning a real `DiscoverResult` (`supportedVersions`, `capabilities`, `_meta['io.modelcontextprotocol/serverInfo']`, `instructions`, `ttlMs`, `cacheScope`) — once that response is spec-correct, a dual-era client stays in modern mode and never falls back.
- **`_meta` lives inside `params`, not as a top-level sibling of `method`/`id`** — and its keys are namespaced (`io.modelcontextprotocol/protocolVersion`, not a flat `protocolVersion`). Confirmed against the real request body: `body.params._meta['io.modelcontextprotocol/protocolVersion']`. An earlier implementation invented a flat top-level `body._meta.protocolVersion` shape that no real client ever sent — designed from reading the spec prose without capturing a real payload to check it against.
- **`Mcp-Name` is required only for `tools/call`/`resources/read`/`prompts/get`** — methods with a `params.name`/`params.uri` to name. Every other method (`ping`, `server/discover`, `tools/list`) correctly omits it; requiring it unconditionally rejects every non-tool-call request from a real client, including the discovery probe above.
- **`resultType` is a fixed enum (`"complete"` / `"input_required"`), never the method name.** An earlier implementation stamped `resultType: method` (e.g. `"tools/call"`), which happened to pass hand-written tests that asserted exactly that, but doesn't match any real spec example — confirmed against `server/discover`'s own documented response shape.
- **When implementing a fix for a client-compatibility bug, capture the real request before designing the fix.** All three bugs above were found by adding temporary request logging, having the failure reproduced once for real, and reading the actual headers/body Claude Code sent — not by re-reading the spec harder. The spec alone would not have revealed that `Mcp-Name` is *conditionally* required, or which exact field path a real client populates.
