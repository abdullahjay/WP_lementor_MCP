...
**What's built today (EMCP-040–043) is still narrower than this table in one respect** — draft/autosave branching (EMCP-045). `{ operations[], document_hash, override_lock? }` → refuses if a different user holds the editor lock (§6.3, EMCP-042, unless overridden), compare-and-swaps against the current hash (§6.4, EMCP-041), validates every operation's element exists before applying any (§7.2's "all operations validate before any apply"), then merges every operation and calls `Document::save()` **once** for the whole batch — always on the **parent**, never an autosave, regardless of publish status. Exposed via the real `edit_elements` MCP tool since EMCP-043 (§7.2) — `server/src/wp/client.ts`'s `editElements()` is its Node-side caller.
