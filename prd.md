...
#### [x] Task 55: `validate_page_spec` and `apply_page_spec`
...
- Done: `validate_page_spec` (`{spec} → {valid, diagnostics, nativeness, raw_ratio}`, no post_id, never writes) and `apply_page_spec` (`{post_id, document_hash, spec, dry_run?, idempotency_key?} → {document_hash, diagnostics, nativeness, raw_ratio, applied, path}`) both registered and live-verified on wp-v4-pro. `apply_page_spec` reuses `PUT /documents/{id}` through a new `op: "replace_tree"` operation shape (sole-operation-in-batch, `plugin/src/Rest/DocumentsController.php`) rather than a parallel write route — full safety-ring parity with `edit_elements`: lock check, hash CAS, autosave branching, snapshot, cache invalidation, ledger, idempotency replay, rollback, all live-confirmed. `document_hash` was added to the input (the earlier draft signature omitted it) since a full-tree replace needs CAS exactly like `edit_elements` does. `server/src/tools/siteProfile.ts`'s `buildSiteProfile()` is the first real assembly of a `SiteProfile` from live `GET /site`+`GET /widgets` calls. Itemised `NATIVENESS_LOW` diagnostics (per offending node, not just the two aggregate numbers) added centrally in `compile.ts`, not the tool layer. **Real bug found via live testing, not unit tests:** `Document::save()` enriches fresh V4 elements with `styles`/`interactions`/`editor_settings`/`version` defaults the compiler doesn't emit, so hashing the pre-save in-memory elements returned a `document_hash` that didn't match a subsequent read — fixed by re-hashing the persisted state after `save()` returns (`DocumentsController::update()`), benefiting `set_settings` too even though it never surfaced there.
